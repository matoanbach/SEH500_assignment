...
#define TIMER_FREQUENCY_HZ 60000000U // 60 MHz
#endif
...
int main(void)
{
    ...
    /* Timer variables used for taking scientific measurement to make Jacky happy */
    uint32_t start_time = 0;
    uint32_t end_time = 0;
    uint32_t elapsed_time_count = 0;
    ...
    // Lets start the timer now for performance measurement
    start_time = FTM_GetCurrentTimerCount(BOARD_TIMER_PERIPHERAL);
    ...
    while (my_counter < 1000)
    {
        ...
        /* Strategy A: Using builtin atan2 to calculate pitch and roll angles from accelerometer data */
        /* You can comment this strategy out and un-comment strategy B to compare the difference */
...
//        }
        ...
        /* Strategy B: Using atan_asm to calculate pitch and roll angles from accelerometer data */
        ...
        /* Calculate yaw angle from magnetometer data */
        yawAngle = (int16_t)(atan_asm((double)magYData, (double)magXData));
        yawAngle %= 360;
    ...
    // Capture end time
    end_time = FTM_GetCurrentTimerCount(BOARD_TIMER_PERIPHERAL);

    // Calculate elapsed time (considering timer overflow)
    elapsed_time_count = (end_time >= start_time) ? (end_time - start_time) : (0xFFFFFFFF - start_time + end_time + 1);

    // Print elapsed time
    PRINTF("\n\nResult: The time elapsed is -> %lu timer counts\n\n", elapsed_time_count);
